...
from data.alphaHF_data import SnapshotData
...
class AlphaPoolBase(metaclass=ABCMeta):
    ...
    @abstractmethod
    def try_new_expr(self, expr: SnapshotExpression) -> float: ...

    @abstractmethod
    def test_ensemble(self, calculator: AlphaCalculator) -> float: ...


class LinearAlphaPool(AlphaPoolBase):
    def __init__(
        self,
        capacity: int,
        calculator: AlphaCalculator,
        ...
        self.size: int = 0
        self.exprs: List[Optional[SnapshotExpression]] = [None for _ in range(capacity + 1)]
    ...
    def force_load_exprs(self, exprs: List[SnapshotExpression]) -> None:
        for expr in exprs:
